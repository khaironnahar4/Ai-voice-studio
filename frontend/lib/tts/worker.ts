...
export async function processTtsJob(job: Job<TtsJobPayload>): Promise<void> {
  ...
  try {
    ...
    await markFailed(
      p.requestId, job,
      new Error(`R2 upload failed: ${err instanceof Error ? err.message : err}`)
    )
    throw err
  }
  ...
  const audioFile = await prisma.audioFile.create({
    data: {
      ...
      elRequestId:     providerName === "elevenlabs"
                         ? result.providerRequestId : null,
      elCharacterCost: providerName === "elevenlabs"
                         ? result.providerCost       : null,
      elModelUsed:     providerName === "elevenlabs"
                         ? result.providerModel      : null,
      elVoiceIdUsed:   providerName === "elevenlabs"
                         ? result.providerVoice      : null,
    },
  })
