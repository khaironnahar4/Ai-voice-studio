...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <nav
          className="w-full lg:max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between"
      ...
      <div
        ...
        className={[
          "w-screen fixed inset-0 z-100 flex flex-col md:hidden",
        ...
        >
          ...
          <nav className="flex flex-col gap-1">
            {NAV_LINKS.map(({ label, href }, i) => (
              <button
                key={href}
                onClick={() => handleNavClick(href)}
                className="flex items-center px-4 py-2 text-sm font-semibold text-left text-vocera-muted hover:text-white hover:bg-white/5 rounded-xl transition-all duration-150"
          ...
          {user ? (
              <div className="flex flex-col gap-3 pt-6 border-t border-white/8">
                ...
                <div
                      onClick={() => signOut()}
                      className="cursor-pointertext-red-300 bg-red-500/10 flex items-center gap-1 px-5 py-2.5 text-sm font-semibold rounded-xl"
                    >
                      <LogOut className="h-4 w-4 mr-2" />
                      Sign out
                    </div>
