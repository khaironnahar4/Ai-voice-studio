...
function WaveformPlayer({ playing }: { playing: boolean }) {
  return (
    <div
      className="flex items-end justify-center gap-[2.5px] h-12"
      aria-hidden="true"
    >
...
export default function Playground() {
  ...
  const [text, setText] = useState(
    "Vocera turns any text into natural, studio-quality voice — in seconds. Try it yourself below.",
  );
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute top-0 left-0 right-0 h-px bg-linear-to-r from-transparent via-vocera-purple/30 to-transparent"
      />
      <div
        aria-hidden="true"
        className="absolute bottom-0 left-0 right-0 h-px bg-linear-to-r from-transparent via-vocera-purple/20 to-transparent"
      />
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          ...
          >
            The AI Voice <span className="text-gradient">Playground</span>
          </h2>
          <p className="reveal opacity-0 translate-y-8 transition-all duration-700 ease-out mt-4 text-vocera-muted text-lg max-w-xl mx-auto">
            No sign-up needed. Paste your text, pick a voice, and hear the
            result instantly.
          </p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label
                    ...
                    className="text-xs font-medium text-vocera-subtle mb-1.5 block"
                  >
                    Emotion
                  </label>
                  <select
                  ...
                  >
                    ...
                    <div
                      className={[
                        ...
                        voice === v.id
                          ? "bg-vocera-purple text-white"
                          : "bg-white/10 text-vocera-muted",
                      ].join(" ")}
                    >
                      {v.name[0]}
                    </div>
                    <div className="font-semibold text-sm text-white leading-none mb-1">
                      {v.name}
                    </div>
                    <div className="text-[11px] text-vocera-subtle">
                      {v.accent}
                    </div>
                    <span
                      className={[
                        ...
                        voice === v.id
                          ? "bg-vocera-purple/30 text-vocera-violet"
                          : "bg-white/6 text-vocera-subtle",
                      ].join(" ")}
                    >
                      {v.style}
                    </span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-white/8 text-vocera-muted hover:text-white transition-colors"
                      >
        ...
        <div className="mx-auto w-full">
          <p className="text-center mx-auto text-xs text-vocera-subtle mt-16">
            3 free conversions without an account ·{" "}
            <Link href="/sign-up" className="text-vocera-violet hover:underline">
              Sign up free
            </Link>{" "}
