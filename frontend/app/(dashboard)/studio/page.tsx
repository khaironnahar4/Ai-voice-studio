...
} from "@/components/studio/VoiceSetting";
...
export default function StudioPage() {
  ...
  const canSubmit =
    text.trim().length > 0 && voice && !overLimit && !isGenerating;
  ...
  async function handleGenerate() {
    ...
    // ── Worker wake-up check ──────────────────────────────────
    try {
      setError(null);
      // UI তে user কে জানানোর জন্য একটা আলাদা state লাগবে (নিচে দেখুন)
      setWakingUp(true);

      const healthRes = await fetch("/api/tts", {
        method: "GET",
        // Vercel এর default timeout 10s — কিন্তু এই call এ 60s লাগতে পারে
        // তাই signal দিয়ে 65s দিচ্ছি
      ...
      if (!healthRes.ok) {
        ...
        setError("Voice server is unavailable. Please try again in a moment.");
        return;
      }
      ...
      setWakingUp(false);
      stopProgressAnimation();
      setError("Could not reach voice server. Please try again.");
      return;
    }
    // ── End wake-up check ─────────────────────────────────────
    ...
    <div className="p-4 md:p-6 lg:p-8 min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          {/* Text area card */}
          <div
            className={`rounded-xl border bg-[#141424] overflow-hidden
          ...
          >
            ...
            <div className="px-5 pb-4 space-y-1.5">
              ...
              <div className="h-0.5 bg-[#282846] rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-300 ${counterBarColor}`}
                  style={{ width: `${charPct}%` }}
                />
              </div>
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[11px] font-mono transition-colors ${counterColor}`}
                >
          ...
          {wakingUp && (
            ...
            >
              <svg
                className="w-4 h-4 text-amber-400 shrink-0 animate-spin"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
              >
              ...
              <p className="text-sm text-amber-300 leading-relaxed">
                Voice server is waking up, please wait 30–50 seconds…
              </p>
            </div>
          )}
          ...
          <button
            ...
            style={{
              background: canSubmit ? "rgb(120,62,246)" : "rgb(26,26,46)",
                ...
                <>
                  ...
                  <span className="flex items-center gap-0.75">
                    {[0, 1, 2, 3, 4].map((i) => (
                      <span
                        key={i}
                        className="w-0.75 rounded-full bg-white/80"
                        style={{
                          height: `${8 + (i % 3) * 4}px`,
                  ...
                  Generating…
                  {genProgress > 0 ? ` ${Math.round(genProgress)}%` : ""}
                </>
              ) : (
                <>
                  <svg
